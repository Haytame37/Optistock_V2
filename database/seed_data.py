...
def seed_database():
    ...
    print("🌱 Démarrage de l'importation vers SQLite (Schéma EARSER)...")
    ...
    if os.path.exists(entrepots_csv):
        ...
        print(f"📦 Importation de {len(df_entrepots)} entrepôts dans la table 'warehouses'...")
    ...
    if temp_files:
        ...
        for f_temp in temp_files:
            ...
            if not os.path.exists(f_hum):
                print(f"⚠️ Fichier d'humidité correspondant introuvable pour {wh_id}. Ignoré.")
                continue
        ...
        print(f"✅ {total_rows} enregistrements IoT fusionnés et importés.")
    else:
        print("⚠️ Aucun fichier de capteurs trouvé.")
        
    conn.commit()
    conn.close()
    print("✅ Base de données SQLite (EARSER) peuplée avec succès !")
